  ...
  */
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      if (!player) {
        return client.sendTime(
          message.channel,
          "❌ | **No servers are playing right now...**"
        );
          }
        if (!player.queue || !player.queue.length || player.queue === 0) {
          return console.log("Errortest: No Queue")
          }
        else {
          let QueueEmbed = new MessageEmbed()
          .setDescription(`Server Queue Status`)
          let copy = client.Manager.players;
          let toArray = Array.from(copy);
          console.log(toArray[[0]]);
          toArray.forEach(function(toArray) {
            console.log(`Server ${toArray[0]} currently has an active queue with ${toArray[1].queue.length} queued songs.`);
            QueueEmbed.addField(`Server ${toArray[0]} currently has an active queue with ${toArray[1].queue.length} queued songs.`)
          });
